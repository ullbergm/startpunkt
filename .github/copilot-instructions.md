...
- GraphQL resources in `graphql/` expose grouped data (`ApplicationGraphQLResource`, `BookmarkGraphQLResource`, `ConfigGraphQLResource`, `I18nGraphQLResource`, `ThemeGraphQLResource`) and always return wrappers from `objects/`.
- `ApplicationGraphQLResource` builds a sorted `ApplicationSpec` list through wrapper classes in `objects/kubernetes`; respect the feature toggles from `application.yaml` and keep `Collections.sort(apps)` before grouping so UI ordering stays deterministic.
...
- Theme queries build responses from `Theme`/`ThemeColors`; always provide both `light` and `dark` palettes because the SPA expects them.
- Front-end code is in `src/main/webui`; `main.jsx` bootstraps Preact components that consume the GraphQL JSON (`groups[].applications[]`, `bookmarks[].links[]`). Component tests live alongside sources with `.test.jsx` using Testing Library + Jest.
...
- Kubernetes-facing tests rely on `@WithKubernetesTestServer`; always register CRDs before seeding resources (see `ApplicationGraphQLResourceTest#before`) and interact through Fabric8 `resource()` helpers instead of raw HTTP.
- GraphQL tests use RestAssured to verify query responses; keep response shapes stable and update the accompanying `.test.jsx` files if the JSON contract changes.
...
- Configuration defaults live in `application.yaml`; any new `@ConfigProperty` entry must be mirrored there and, if exposed to the UI, surfaced via `ConfigGraphQLResource`.
...
- Follow conventional commit format: `type(scope): description` (e.g., `feat(api): add theme query`, `fix(ui): correct bookmark sorting`).
...
- GraphQL endpoints must have RestAssured tests validating query/mutation contracts.
...
- GraphQL queries return typed objects from `objects/` package, never primitives or raw maps.
