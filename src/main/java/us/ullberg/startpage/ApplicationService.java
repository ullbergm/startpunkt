...
public class ApplicationService {
    @Timed(value = "startpage.kubernetes.hajimari", description = "Get a list of hajimari applications")
    public List<Application> retrieveHajimariApplications() {
        ...
        try (final KubernetesClient client = new KubernetesClientBuilder().build()) {
            ...
            GenericKubernetesResourceList list = client.genericKubernetesResources(resourceDefinitionContext)
                    .inAnyNamespace().list();

            List<Application> apps = list.getItems().stream().map(item -> {
                ...
                return new Application(name, group, icon, iconColor, url, info, targetBlank, location, enable);
            }).toList();
    ...
    private String getUrl(GenericKubernetesResource item) {
        ...
        if (annotations.containsKey("hajimari.io/url"))
    ...
    private String getIcon(GenericKubernetesResource item) {
        ...
        if (annotations.containsKey("hajimari.io/icon"))
    ...
    private String getInfo(GenericKubernetesResource item) {
        ...
        if (annotations.containsKey("hajimari.io/info"))
    ...
    private String getGroup(GenericKubernetesResource item) {
        ...
        return (spec.containsKey("group") ? spec.get("group").toString() : item.getMetadata().getNamespace())
                .toLowerCase();
    }
    ...
    private Boolean getTargetBlank(GenericKubernetesResource item) {
        ...
        return spec.containsKey("targetBlank") ? Boolean.parseBoolean(spec.get("targetBlank").toString()) : null;
    }
